...
namespace LoginRadius\Ciam\Controller\Auth;

use Magento\Framework\App\Action\NotFoundException;
use Magento\Framework\Controller\ResultFactory;
use LoginRadius\Ciam\Controller\Auth\Customhttpclient;
...
class Index extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        $this->_helperActivation = $this->_objectManager->get('LoginRadius\Ciam\Model\Helper\Data');
       ...
       // $this->_helperCustomerRegistration = $this->_objectManager->get('LoginRadius\\' . $this->_helperActivation->getAuthDirectory() . '\Model\Helper\Data');
        ...
        if ($this->_helperActivation->siteApiKey() != '' && !defined('LR_API_KEY')){
            define('LR_API_KEY', $this->_helperActivation->siteApiKey());
        }
        if ($this->_helperActivation->siteApiSecret() != '' && !defined('LR_API_SECRET')){
            ...
            try {
                ...
                if (isset($userProfileData->Uid)) {
                    ...
                    elseif ($this->_customerSession->isLoggedIn()) {
                        return $this->redirectLoginPage('ciam/accounts/linking');
                    }
                    else {
    ...
    function saveEntitiesData($userProfileData, $isActive = 1) {
        ...
        $saveMailInDb = $this->_helperActivation->saveMailInDb();
    ...
    */
    ...
    function setCustomerLoggedIn($customer, $userProfileData, $is_new = false) {
        ...
        $this->_customerSession->setCurrentProvider($userProfileData->Provider);    
        $emailVerified = isset($userProfileData->EmailVerified) ? $userProfileData->EmailVerified : true;
    ...
    function redirectionUrl($is_new) {
        $redirection = $this->_helperActivation->loginRedirection();
        $request = $this->_request->getParams();
        if (isset($request['redirect_to']) && !empty($request['redirect_to'])) {
            ...
            switch ($redirection) {
                case 'custom':
                    $redirection = $this->_helperActivation->customLoginRedirection();
                    break;
                case 'account':
